...
Gli *attori*, utenti e amministratori, saranno interfacciati con una **pagina web**, che comunica con un **server** che gestisce i dati. La pagina web è accessibile da ogni browser abbastanza aggiornato.
...
Ad ogni momento l'esecuzione **single threaded** valuta *una riga* di codice.
...
x = "Ciao, mondo!"; // assegnamento di una stringa
x = saluto; // assegnamento di una variabile (copia)
...
`Math.cos(n)`|`Math.sin(n)`|`Math.exp(n)`|`Math.log(n)`|`Math.pow(b, e)`|`Math.sqrt(n)`
---|---|---|---|---|---
</div>
</div>
...
I due esempi sono equivalenti. Esiste anche il costrutto `switch` per controllare il valore di una variabile e l'operatore ternario `(cond) ? /*true*/ : /*false*/`:
...
Sono funzioni che vengono _passate come parametro_ ad un'altra funzione, e che vengono chiamate in un momento successivo.
