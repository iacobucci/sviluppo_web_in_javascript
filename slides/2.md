...
La sintassi è `<nome-tag att-1="val-1" ... att-n="val-n">testo</nome-tag>`.

---

## Rendering del DOM

Ogni tipo di elemento ha un comportamento di default.
...
- **creare un oggetto**: `let obj = {}`, viene _istanziato_. `obj` è il _riferimento_.
...
```javascript
let arr = ["elemento_1", "elemento_2", "elemento_3"];
...
Il browser rileva gli _eventi_ ai quali possiamo agganciare delle funzioni.
...
Definiamo la funzione (di callback) che sarà eseguita quando l'evento si verifica.
