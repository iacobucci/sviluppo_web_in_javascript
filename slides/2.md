...
3) **interpreta** e il codice Javascript viene eseguito.
...
- **console** per inviare comandi Javascript
...
Scriviamo un file HTML, ha una struttura ad albero e ogni nodo è un *elemento*.
...
La sintassi è `<nome-tag att-1="valore-1" ... att-n="valore-n">testo</nome-tag>`.
...
La pagina è *statica*, mostra solo i dati scaricati con la prima richiesta HTTP.
...
- `document` rappresenta il DOM.
	...
	- `document.getElementById()` trova un elemento con un id.
...
- **ottenere una proprietà** `obj.prop` o `obj["prop"]`. **Chiavi** `number` e `string`.

---

### Prototype

Gli oggetti hanno un *prototype* da cui **ereditano** proprietà e metodi.
...
Sono *oggetti ordinati*, **iterables** quindi si possono scorrere con un ciclo.
...
Sono un tipo di dato *primitivo*, **immutabili**. `String()` invece è un oggetto.
...
- Javascript usando solo API DOM o *librerie*.
